...
void animacion()
{
	...
	if (circuito)
	{
		if (recorrido1)
		{
			...
			{
				recorrido1 = false;
				recorrido2 = true;  //ya llegas al punto 2
			}
		}
		if (recorrido2)
		{
			rotKit = 90;           //gira 90 grados el carrito
			movKitX++;
			if (movKitX > 125)
			{
				recorrido2 = false;
				recorrido3 = true; //llegas al punto 3

			}
		}
		if (recorrido3)
		{
			rotKit = 205;		//rota el carrito de tal manera que apunte al punto 4
			movKitZ -= 2.4;		//punto final de la carretera
			movKitX--;
			if (movKitX<0)
			{
				recorrido3 = false;
				recorrido5 = true; //llegas punto 4
			}
		}
		/*
		if(recorrido4)
